...
import { previewRows, workbookToXlsx } from '../../utils/workbook'
...
watch(() => route.query.type, (id) => {
  store.setType(TABLE_TYPES.some((t) => t.id === id) ? id : 'shared')
  if (!previewMode.value) editorId.value = ''
}, { immediate: true })
watch(() => [route.query.preview, route.query.id], ([preview, id]) => {
  if (String(preview) === '1' && id) editorId.value = String(id)
}, { immediate: true })

function previewOf(t) {
  return previewRows(t.workbook)
}
...
<template>
        ...
        <div v-else-if="store.viewMode === 'card'" class="gal-grid">
          ...
          >
            ...
            <div class="tc-preview">
              <div v-if="!previewOf(t).length" class="tc-empty">暂无预览</div>
              <table v-else>
                <tbody>
                  <tr v-for="(row, i) in previewOf(t)" :key="i">
                    <template v-if="i === 0">
                      <th v-for="(c, j) in row" :key="j">{{ c }}</th>
                    </template>
                    <template v-else>
  ...
  <AppModal :visible="addModal.visible" title="添加表格" icon="g-1840980a" :width="420" @update:visible="(v) => { addModal.visible = v }">
    <div class="fm-field">
      <label>所属目录</label>
  ...
  <TableEditor :table-id="editorId" :preview="previewMode" @close="editorId = ''" />
</template>
