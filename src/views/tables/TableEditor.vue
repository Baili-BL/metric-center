...
import { bootUniver, openWorkbook, registerRibbonExtras, setWorkbookReadonly, snapshotWorkbook } from '../../utils/univer-boot'
import {
...
async function ensureBoot() {
  ...
  await nextTick()
  try {
    api = await bootUniver(hostRef.value || 'univerHost')
    if (!ribbonReady && !props.preview) {
      registerRibbonExtras(api, {
        onPivot: openPivotWizard,
        toast: (msg) => Message.info(msg),
      })
      ribbonReady = true
    }
  } catch (err) {
    bootErr.value = err?.message || '表格引擎加载失败，请刷新后重试'
  } finally {
    booting.value = false
  }
}
...
function capture() {
  if (props.preview || !api || !props.tableId) return null
  const snap = snapshotWorkbook(api)
  if (snap) store.saveWorkbook(props.tableId, snap)
  return snap
}
...
async function loadCurrent() {
  ...
  await openWorkbook(api, t.workbook || { id: t.id, name: t.title, sheetOrder: [], sheets: {} })
  if (props.preview) setWorkbookReadonly(api)
  setTimeout(() => window.dispatchEvent(new Event('resize')), 60)
}

function dismissUniverPopups() {
  document.querySelectorAll('.univer-popup').forEach((el) => { el.style.display = 'none' })
}
...
function onPreview() {
  ...
  capture()
  const url = `${location.origin}/tables?type=${encodeURIComponent(store.currentType)}&id=${encodeURIComponent(t.id)}&preview=1`
...
function liveSheet() {
  try { return api?.getActiveWorkbook?.()?.getActiveSheet?.() } catch { return null }
}
function liveSheetName() {
  const sheet = liveSheet()
  try {
    if (sheet?.getSheetName) return sheet.getSheetName()
    if (sheet?.getName) return sheet.getName()
  } catch { /* */ }
  const list = listWorkbookSheets(table.value?.workbook)
  return list[0]?.name || '数据'
}
function liveSelectionRange() {
  const sheet = liveSheet()
  if (!sheet) return null
  try {
    let range = sheet.getActiveRange ? sheet.getActiveRange() : null
    if (!range && sheet.getSelection) {
      const sel = sheet.getSelection()
      range = sel?.getActiveRange ? sel.getActiveRange() : null
    }
    if (!range) return null
    const sr = range.getRow ? range.getRow() : 0
    const sc = range.getColumn ? range.getColumn() : 0
    const er = range.getLastRow ? range.getLastRow() : (range.getNumRows ? sr + range.getNumRows() - 1 : sr)
    const ec = range.getLastColumn ? range.getLastColumn() : (range.getNumColumns ? sc + range.getNumColumns() - 1 : sc)
    if (er === sr && ec === sc) return null
    return { startRow: sr, startColumn: sc, endRow: Math.max(er, sr), endColumn: Math.max(ec, sc) }
  } catch { return null }
}
...
async function insertPivotSheet(name) {
  ...
  try {
    if (wb.insertSheet) {
      ...
      if (created?.setName) created.setName(name)
      return created || liveSheet()
    }
  } catch { /* */ }
  try { if (wb.create) return wb.create(name) } catch { /* */ }
  return null
}
...
function writeUniverBlock(startRow, startCol, matrix, headerRows) {
  ...
  try {
    const wb = api?.getActiveWorkbook?.()
    const named = pivot.outName && wb?.getSheetByName?.(pivot.outName)
    if (named) {
      if (wb.setActiveSheet) wb.setActiveSheet(named)
      sheet = named
    }
  } catch { /* */ }
  if (!sheet || !matrix.length || !matrix[0].length) return false
  const h = matrix.length
  const w = matrix[0].length
  try {
    const range = sheet.getRange(startRow, startCol, h, w)
    if (range?.setValues) {
      range.setValues(matrix)
      if (headerRows) {
        try {
          const head = sheet.getRange(startRow, startCol, headerRows, w)
          if (head.setBackgroundColor) head.setBackgroundColor('#E8F3FF')
          else if (head.setBackground) head.setBackground('#E8F3FF')
          if (head.setFontWeight) head.setFontWeight('bold')
        } catch { /* */ }
      }
      return true
    }
  ...
  try {
    for (let r = 0; r < h; r++) {
      for (let c = 0; c < w; c++) {
        ...
        if (cell?.setValue) cell.setValue(matrix[r][c])
      }
    }
    return true
  } catch { return false }
...
async function refreshPivotTable() {
  ...
  if (!ok) {
    ...
    if (pack) {
      ...
      await openWorkbook(api, table.value.workbook)
    }
  } else {
    capture()
  }
  ...
  <Teleport to="body">
    <div class="editor-root" :class="{ open, 'is-preview': preview }" :aria-hidden="!open">
      <div class="editor-bar">
        ...
        <span class="editor-title">{{ table?.title || '未命名表格' }}{{ preview ? '（预览）' : '' }}</span>
      ...
      <div class="editor-stage">
        <div id="univerHost" ref="hostRef"></div>
        ...
        <div v-if="booting || bootErr" class="sheet-boot" :class="{ hidden: !booting && !bootErr }">
          <div class="load-box">
            <div v-if="!bootErr" class="load-spin"></div>
            <span>{{ bootErr || '表格引擎加载中…' }}</span>
          </div>
        </div>
      </div>
    </div>
