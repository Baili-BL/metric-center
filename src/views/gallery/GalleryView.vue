...
const drawerId = computed(() => (creating.value ? '' : String(route.params.id || '')))
const drawerOpen = computed(() => creating.value || !!String(route.params.id || ''))
...
watch(() => String(route.params.id || ''), (id) => {
  ...
  const c = store.getChart(id)
  if (!c) {
    Message.error('图表不存在或已删除')
...
function chartLink(c) {
  return { name: 'gallery', params: { id: c.id } }
}
function openNew() {
  closeMenu()
  const go = () => { creating.value = true }
  if (route.params.id) router.push({ path: '/gallery' }).then(go)
  else go()
}
function openEdit(c) {
  if (!c) return
  ...
  router.push(chartLink(c))
}
function closeDrawer() {
  creating.value = false
  if (route.params.id) router.push({ path: '/gallery' })
}
function isFavOn(c) {
  return isFav(c)
}
...
<template>
  ...
  <Teleport to="body">
    <div v-if="drawerOpen" class="builder-drawer-root open is-ready">
      <div class="builder-drawer" role="dialog" aria-modal="true" aria-label="新建图表">
        <ChartBuilder :chart-id="drawerId" mode="drawer" @saved="closeDrawer" @close="closeDrawer" />
      </div>
    </div>
  </Teleport>
